...
namespace NiceDreamers.Windows.Utilities
{
    ...
    ///     Supports events of type EventHandler where the event arguments derive from System.EventArgs
    /// </summary>
    public class CommandOnEventHelper : DependencyObject
    {
        ...
        private static readonly MethodInfo HandlerMethodInfo = typeof (CommandOnEventHelper).GetMethod(
            "OnEventHandler", BindingFlags.NonPublic | BindingFlags.Static);
        ...
        /// </summary>
        public static void AssociationEntryPropertyChangedCallback(DependencyObject sender,
            DependencyPropertyChangedEventArgs e)
        {
            if (null == sender)
            {
                throw new ArgumentNullException("sender",
                    @"Unable to configure event-to-command mapping with a null dependency object");
            }
            ...
            if (null != e.OldValue)
            {
                ...
                if (entry != null)
                {
                    EventInfo eventInfo = GetEventInfo(sender, entry.Event);
            ...
            if (null != e.NewValue)
            {
                ...
                {
                    EventInfo eventInfo = GetEventInfo(sender, entry.Event);
                    Delegate handlerDelegate = Delegate.CreateDelegate(eventInfo.EventHandlerType, null,
                        HandlerMethodInfo);
        ...
        private static void OnEventHandler(object sender, EventArgs e)
        {
            ...
            if (obj != null)
            {
                ...
                if (entry != null && entry.Command.CanExecute(e))
                {
        ...
        private static EventInfo GetEventInfo(object sender, string eventName)
        {
            ...
            if (null == eventInfo)
            {
                throw new InvalidOperationException(string.Format(CultureInfo.InvariantCulture,
                    Resources.CommandOnEventException, sender.GetType().FullName, eventName));
            }
