...
namespace NiceDreamers.Windows.Controls
{
    ...
    public class PromptControl : ContentControl
    {
        ...
        public static readonly DependencyProperty TextProperty =
            DependencyProperty.Register("Text", typeof (string), typeof (PromptControl),
                new PropertyMetadata(string.Empty));

        public static readonly DependencyProperty MinimumStateDurationProperty =
            DependencyProperty.Register("MinimumStateDuration", typeof (TimeSpan), typeof (PromptControl),
                new PropertyMetadata(TimeSpan.FromSeconds(DefaultMinimumStateDurationSeconds)));

        public static readonly DependencyProperty StateTransitionDelayProperty =
            DependencyProperty.Register("StateTransitionDelay", typeof (TimeSpan), typeof (PromptControl),
                new PropertyMetadata(TimeSpan.FromSeconds(DefaultStateTransitionDelaySeconds)));

        public static readonly DependencyProperty StateProperty =
            DependencyProperty.Register("State", typeof (PromptState), typeof (PromptControl),
                new PropertyMetadata(PromptState.Hidden, OnStateChanged));
        ...
        /// </summary>
        private PromptState? currentVisualState;
        ...
        {
            DefaultStyleKey = typeof (PromptControl);
        ...
        {
            get { return (TimeSpan) GetValue(MinimumStateDurationProperty); }

            set { SetValue(MinimumStateDurationProperty, value); }
        }
        ...
        {
            get { return (TimeSpan) GetValue(StateTransitionDelayProperty); }

            set { SetValue(StateTransitionDelayProperty, value); }
        }
        ...
        {
            get { return (PromptState) GetValue(StateProperty); }
        ...
        public override void OnApplyTemplate()
        {
            ...
            GoToVisualState(PromptState.Hidden, false, false);
        }
        ...
        private void GoToVisualState(PromptState state, bool useTransitions, bool delayTransition)
        {
            ...
            bool isStateTransition = !currentVisualState.HasValue || (state != currentVisualState.Value);
            minimumStateDurationTimer.Stop();
            stateTransitionDelayTimer.Stop();
            ...
            if (currentVisualState.HasValue)
            {
                ...
                // the current state
                TimeSpan timeInCurrentState = currentTime.Subtract(timeEnteredVisualState);
                TimeSpan timeRemaining = MinimumStateDuration.Subtract(timeInCurrentState);

                if (timeRemaining.CompareTo(zeroDuration) > 0)
                {
                    // If we need to spend more time in current state before transitioning,
                    // defer transition until enough time passes.
                    minimumStateDurationTimer.Interval = timeRemaining;
                    minimumStateDurationTimer.Start();
                    return;
                }
            }
            ...
            if (delayTransition)
            {
                ...
                stateTransitionDelayTimer.Interval = StateTransitionDelay;
                stateTransitionDelayTimer.Start();
                return;
            }
            ...
            if (VisualStateManager.GoToState(this, GetVisualStateName(state), useTransitions))
            {
                ...
                timeEnteredVisualState = currentTime;
            }
        }

        private void GoToVisualState(PromptState state, bool useTransitions)
        {
            GoToVisualState(state, useTransitions, true);
        }
        ...
        {
            stateTransitionDelayTimer.Stop();
            GoToVisualState(State, true, false);
        }
    }
}